...
pub mod emulator {
    ...
    use std::thread;
    use std::time::Instant;
    pub fn fetch_decode(memory: &[u8; 4096], pc: &mut u16) -> [u16; 4] {
...
fn main() -> io::Result<()> {
    ...
    let mut f = File::open("tests/Chip-8 Demos/Sierpinski [Sergey Naydenov, 2010].ch8")?;
    let mut pc: u16 = 512;
    ...
    emulator::load_fonts(&mut memory);
    ...
    'running: loop {
        let now = Instant::now();
        let codes = emulator::fetch_decode(&mut memory, &mut pc);
        println!("{:?}, {:x}, ({:x})", codes, pc - 2, pc - 514);
        emulator::execute(
            &codes,
            &mut memory,
            &mut registers,
